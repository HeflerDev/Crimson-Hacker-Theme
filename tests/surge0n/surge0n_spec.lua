require("plenary.reload").reload_module("surge0n", true)
local surge0n = require("surge0n")
local default = surge0n.config
...
describe("tests", function()
  it("works with default values", function()
    surge0n.setup()
    assert.are.same(surge0n.config, default)
  end)
  ...
  it("works with config overrides", function()
    ...
    surge0n.setup({ undercurl = false, underline = false })
    assert.are.same(surge0n.config, expected)
  end)
  ...
  it("does not set terminal colors when terminal_colors is false", function()
    clear_term_colors()
    surge0n.setup({ terminal_colors = false })
    surge0n.load()
  ...
  it("sets terminal colors when terminal_colors is true", function()
    clear_term_colors()
    surge0n.setup({ terminal_colors = true })
    surge0n.load()
